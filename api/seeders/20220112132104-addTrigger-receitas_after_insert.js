...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    return sequelize.query(
      ` CREATE DEFINER = CURRENT_USER TRIGGER ${database}.receitas_AFTER_INSERT AFTER INSERT ON receitas FOR EACH ROW BEGIN` +
      ...
      ` SET @somaReceitas = (SELECT sum(valor) FROM ${database}.receitas WHERE YEAR(data) = @ano AND MONTH(data) = @mes AND usuario_id = @usuario);` +
      
      ` IF (@mesExiste = FALSE OR @anoExiste = FALSE OR @usuario = FALSE) THEN` +
    ...
    async down (queryInterface, Sequelize) {
      ...
      return sequelize.query(
        `DROP TRIGGER IF EXISTS ${database}.receitas_AFTER_INSERT;`
        );
    },
};
